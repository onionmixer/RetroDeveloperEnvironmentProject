/* Phase C: 이 모듈의 code+rodata 를 BANK_01 으로 분리해 main code ≤16K 유지.
 * (Konami 매퍼의 boot default 가 BANK_01 을 $8000-$BFFF 에 mount 하므로
 * 호출 시 별도 banked_call thunk 없이 직접 호출 가능 — prototype_06 검증.)
 * SDCC 빌드 / Phase B (ASCII16) 빌드는 `#pragma bank` 를 무시. */
#ifdef __Z88DK
#pragma bank 1
#endif
