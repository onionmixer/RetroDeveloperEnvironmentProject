...
#if !defined(MSX_BUILD) && !defined(__SDCC)
...
void ap_uncompress(const uint8_t* dst, const uint8_t* src)
{
    ...
    apultra_decompress(src, (uint8_t*)dst, n, 32 * 24, 0, 0);
}
uint8_t* VIDEO_MEMORY_ADDRESS = 0;
#else
uint8_t* VIDEO_MEMORY_ADDRESS = (uint8_t*)0x1800;
#endif
...
#endif
{
    ...
    /* ASCII16 64K cart: openMSX RomAscii16kB::reset 가 bank 0 을 양쪽
       window ($4000 + $8000) 에 mirror 시킨다. main 자체는 bank 0 ($43B9) 에
       있어 안전하지만, 모든 ubox/mplayer/spman/ap 라이브러리는 bank 1
       ($8000-$BFFF) 에 거주. 그 어떤 호출보다 먼저 bank 1 을 $8000 window
       에 mount 해야 한다. ASCII16 trigger: write to $7000. */
    *(volatile unsigned char*)0x7000 = 1;
#endif
#if !defined(MSX_BUILD) && !defined(__SDCC)
...
#endif
#if defined(MSX_BUILD)
    /* Phase B: LDIR-copy the AKM player blob from cart ROM to its
       runtime location ($D500) before any mplayer_* call. Must run
       once before mplayer_init/mplayer_init_effects. */
    {
