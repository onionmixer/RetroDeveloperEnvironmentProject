#!/usr/bin/env bash
# Prototype 01 Apple II ProDOS launcher
#
# Flow:
#  1) (optional) compile.sh all
#  2) run AppleWin with D1 boot + prepared D2 program disk
...
APPLEWIN="${APPLEWIN:-}"
BOOT_DISK="${BOOT_DISK:-}"
PROGRAM_DISK="${PROGRAM_DISK:-$SCRIPT_DIR/build/prototype_01_appleii_prodos.po}"
...
echo "[run] D2 disk: $PROGRAM_DISK"

echo "Starting AppleWin (ProDOS)"
