#!/usr/bin/env bash
# 01_hello — MSX-DOS2 .COM launcher in openMSX
# Builds a fresh /tmp working disk from the project's MSX-DOS2 boot disk
# at every run, injects the latest 01_hello.COM, then boots openMSX.
...
DOS_NAME="HELLO"   # 8.3 uppercase command name shown at MSX-DOS prompt
...
# Locate the COM (with or without .COM extension)
COM=""
...
# Stage a disposable working disk in /tmp (prototype_05 pattern)
WORK_DIR="${WORK_DIR:-/tmp/01_hello_dos}"
mkdir -p "$WORK_DIR"
...
echo "Starting 01_hello in MSX-DOS2..."
echo "  COM:     $COM"
echo "  Disk:    $WORK_DISK"
