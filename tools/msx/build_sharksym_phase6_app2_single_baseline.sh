...
WORK="${WORK:-/tmp/sharksym_phase6_app2_single_baseline}"
OUT="$WORK"
INC_LC="$WORK/include_sharksym_lc"
REPORT="${REPORT:-$WORK/app2_single_baseline_report.json}"
TARGET="${TARGET:-APP2}"
CFG_DIR="${CFG_DIR:-$HITECH/examples/sharksym/2HELLO}"
...
echo "[phase6-app2-single] compile objects"
read -r -a SRC_ARR <<<"$SRC_LIST"
for src_name in "${SRC_ARR[@]}"; do
  src_path="$(resolve_src "$src_name")" || { echo "[phase6-app2-single] source not found: $src_name" >&2; exit 1; }
  ...
  if [[ "$ext" == "c" ]]; then
    ...
    echo "[phase6-app2-single] unsupported source extension: $src_name" >&2
    exit 1
  fi
  shopt -u nocasematch
done
...
if [[ -n "${LIB_LIST// /}" ]]; then
  ...
  for lib_name in "${LIB_ARR[@]}"; do
    lib_path="$(resolve_lib "$lib_name")" || { echo "[phase6-app2-single] library not found: $lib_name" >&2; exit 1; }
    libs+=("$lib_path")
  done
fi

OUT_COM="$OUT/${TARGET}_BANK00.COM"
OUT_MAP="$OUT/${TARGET}_BANK00.map"
echo "[phase6-app2-single] link $(basename "$OUT_COM")"
link_cmd=("$BIN/linq3" "-z" "-ptext=100H${PSECT_CFG}" "-m$OUT_MAP" "-o$OUT_COM")
link_cmd+=("${objs[@]}")
if [[ ${#libs[@]} -gt 0 ]]; then link_cmd+=("${libs[@]}"); fi

set +e
"${link_cmd[@]}" >/tmp/phase6_app2_single_link.log 2>&1
link_rc=$?
set -e
if [[ $link_rc -ne 0 && ! -s "$OUT_COM" ]]; then
  echo "[phase6-app2-single] link failed rc=$link_rc" >&2
  sed -n '1,120p' /tmp/phase6_app2_single_link.log >&2 || true
  exit 1
fi

cat >"$REPORT" <<EOF
{
  "tool": "build_sharksym_phase6_app2_single_baseline",
  "status": "pass",
  "target": "$TARGET",
  "out_dir": "$OUT",
  "cfg_dir": "$CFG_DIR",
...
EOF
echo "[phase6-app2-single] report=$REPORT status=pass"
echo "[phase6-app2-single] done: $OUT"
